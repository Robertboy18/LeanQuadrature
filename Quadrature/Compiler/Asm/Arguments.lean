...
/-!
# Assembly builtin and external arguments

Builtin-argument evaluation for assembly, after the generic rules of `common/Events.v`.
Read `EvalBuiltinArg` first: processor registers are read directly, and split-word
arguments evaluate both halves. `eval_builtin_args_iff` shows the executable evaluator
sound and complete for the relation. The second half does the same for external-call
arguments under the ELF64 convention, reading Mach locations through `pregOf`.
...
/-- One argument of an external call: a Mach register read through `pregOf`, or an outgoing
stack slot loaded at four times its abstract offset. -/
inductive ExternalArg (registers : Regset) (memory : Mem) (stack : Val) :
    LTL.Location → Val → Prop where
  | register (reg) : ExternalArg registers memory stack (.R reg) (registers (pregOf reg))
...
abbrev ExternalArgs (registers : Regset) (memory : Mem) (stack : Val)
    (signature : Signature) :=
  List.Forall₂ (ExternalArg registers memory stack) (LTL.locArguments signature)
...
def externalArg (registers : Regset) (memory : Mem) (stack : Val) :
  ...
  | .R reg => some (registers (pregOf reg))
...
def externalArgs (registers : Regset) (memory : Mem) (stack : Val)
  ...
  externalArgList registers memory stack (LTL.locArguments signature)
