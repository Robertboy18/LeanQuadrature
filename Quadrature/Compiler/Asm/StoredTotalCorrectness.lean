import Quadrature.Compiler.Asm.Determinism
import Quadrature.Compiler.Asm.StoredPrograms

/-!
# All executions of the ten imported assembly programs

Total correctness of the ten programs in the assembly semantics: for n = 1..10, every
...
`not_infinite`. Execution uses the `RA` register and stack memory, with no return-address
oracle. The theorems hold for any external-call relation, because the checked executions
call only numeric annotations, whose events are fixed by their arguments.
...
theorem reachable_can_finish (n : Nat) (hlo : 1 ≤ n) (hhi : n ≤ 10)
    ...
    (hrun : Steps (Imported.program n).globalenv
      start trace state) :
...
theorem progress (n : Nat) (hlo : 1 ≤ n) (hhi : n ≤ 10)
    ...
    FinalState state Integers.Int.zero ∨
      ∃ emitted next, Step (Imported.program n).globalenv
        state emitted next :=
...
theorem maximal_execution (n : Nat) (hlo : 1 ≤ n) (hhi : n ≤ 10)
    ...
    (hrun : Steps (Imported.program n).globalenv
      start trace state)
    (hstop : ∀ emitted next, ¬ Step (Imported.program n).globalenv
      state emitted next) :
