...
import Quadrature.Compiler.Cminor.Imported
import Quadrature.Compiler.Cminor.ProgramExecution

/-!
# Executions of the ten Cminor programs

`execution_checked` runs the checker on `Imported.program n` for `stepCount n` steps,
n = 1..10, by kernel evaluation. Each run initializes every global, calls the internal
polynomial callback, emits its binary64 result, and exits 0. The emitted value is
`Clight.StoredPolynomial.result n`, the value computed at the Clight stage, and inherits
its integral-error bound (`execution_accuracy`). `execution` turns the checked run into
a `Steps` derivation.
...
namespace Quadrature.Cminor.StoredPrograms
...
/-- `stepCount n` is the number of checker steps of the n-node Cminor program: 30 plus 44 per
node. -/
def stepCount (n : Nat) : Nat := 44 * n + 30
...
end Quadrature.Cminor.StoredPrograms
