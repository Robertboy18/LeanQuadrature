import Quadrature.Compiler.CminorSel.ProgramExecution

/-!
# Uniqueness and termination of checked CminorSel executions
...
theorem execute_step_agrees (hexternal : ExternalExecutorAgrees external ge)
  ...
  | store f chunk address args a k sp env memory values value pointer memory' ha hv hp hm =>
      simpa [executeStep, eval_expr_list_complete ha, eval_expr_complete hv, hp, hm]
        using h.symm
  | call f result signature a args k sp env memory vf values fd hf hv hfd hsig =>
      simpa [executeStep, eval_expr_or_symbol_complete hf, eval_expr_list_complete hv, hfd, hsig]
        using h.symm
  | tailcall f signature a args k sp env memory vf values fd memory' hf hv hfd hsig hm =>
      simpa [executeStep, eval_expr_or_symbol_complete hf, eval_expr_list_complete hv,
        hfd, hsig, freeStack, hm] using h.symm
  | builtin f result ef args k sp env memory values emitted value memory' hv hext =>
      simp only [executeStep, eval_builtin_args_complete hv, bind, Option.bind_some,
      ...
      exact ⟨rfl, rfl⟩
  | branch f condition yes no k sp env memory result hb =>
      simpa [executeStep, eval_cond_expr_complete hb] using h.symm
  | switch f a k sp env memory depth hdepth =>
      simpa [executeStep, eval_exit_expr_complete hdepth] using h.symm
