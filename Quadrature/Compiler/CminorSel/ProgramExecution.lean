import Quadrature.Compiler.CminorSel.Execution
...
def initialState (program : Program) : Option State := do
  ...
  if function.signature = mksignature [] .Xint cc_default then
    pure (.Callstate function [] .Kstop memory)
  else none
...
def exitStatus : State → Option Integers.Int
  | .Returnstate (.Vint status) .Kstop _ => some status
  | _ => none
...
theorem exit_status_sound {state : State} {status : Integers.Int}
  ...
  | Running => cases h
  | Callstate => cases h
  | Returnstate value continuation memory =>
      cases value <;> cases continuation <;> cases h
      constructor
