...
/-!
# Executions of the ten CminorSel programs

`execution_checked` runs the checker on `Imported.program n` for `stepCount n` steps,
n = 1..10, by kernel evaluation. Each run initializes all 73 globals, executes the
selected accessors, integrator, internal polynomial, and caller, emits its binary64
result, and exits 0. The emitted value is `Clight.StoredPolynomial.result n` and
inherits its integral-error bound (`execution_accuracy`). `execution` turns the checked
run into a `Steps` derivation.
...
/-- `stepCount n` is the number of checker steps of the n-node CminorSel program: 30 plus 44 per
node, the same count as in Cminor. -/
