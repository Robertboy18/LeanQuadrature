import Quadrature.Compiler.LTL.Execution
...
`fuel` checker steps, and reads the exit status from register `AX`. `initialState`
allocates every global and resolves the entry symbol. `execute_program_sound` turns a
successful result into an initial state, a `Steps` run with that trace, and a final state
with that status.
...
def initialState (program : Program) : Option State := do
  ...
  if function.signature = mksignature [] .Xint cc_default then
    pure (.Callstate [] function Locset.empty memory)
  else none
...
theorem exit_status_sound {state : State} {status : Integers.Int}
  ...
  cases state with
  | Running | Block | Callstate => cases h
