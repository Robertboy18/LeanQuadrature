...
/-!
# RTL builtin arguments

Relational and executable evaluation of `BuiltinArg`, after the generic
`eval_builtin_arg` rules in `common/Events.v`. `EvalBuiltinArg ge sp rs m arg v` says
`arg` evaluates to `v`. Registers are read directly, and split-word arguments
...
/-- `EvalBuiltinArg ge sp rs m arg v` says the builtin argument `arg` evaluates to `v` with stack
pointer `sp`, register file `rs`, and memory `m`. -/
inductive EvalBuiltinArg (ge : Genv) (stack : Val) (env : Regset) (memory : Mem) :
...
/-- `EvalBuiltinArgs ge sp rs m args vs` evaluates the builtin arguments pointwise to `vs`. -/
abbrev EvalBuiltinArgs (ge : Genv) (stack : Val) (env : Regset) (memory : Mem) :=
  List.Forall₂ (EvalBuiltinArg ge stack env memory)

/-- `evalBuiltinArg ge sp rs m arg` computes the value of one builtin argument, or fails. -/
def evalBuiltinArg (ge : Genv) (stack : Val) (env : Regset) (memory : Mem) :
...
/-- `evalBuiltinArgs ge sp rs m args` computes the values of the builtin arguments in order,
failing if any fails. -/
def evalBuiltinArgs (ge : Genv) (stack : Val) (env : Regset) (memory : Mem) :
...
variable {ge : Genv} {stack : Val} {env : Regset} {memory : Mem}

/-- If `EvalBuiltinArg ge sp rs m arg v` holds, then `evalBuiltinArg ge sp rs m arg` returns
`some v`. -/
...
/-- If `evalBuiltinArg ge sp rs m arg` returns `some v`, then `EvalBuiltinArg ge sp rs m arg v`
holds. -/
