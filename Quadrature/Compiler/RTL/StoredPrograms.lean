...
import Quadrature.Compiler.RTL.Imported
import Quadrature.Compiler.RTL.ProgramExecution

/-!
# Executions of the ten RTL programs

`execution_checked` runs the checker on `Imported.program n` for `stepCount n` steps,
n = 1..10, by kernel evaluation. Each run initializes all 73 globals, executes the RTL
...
namespace Quadrature.RTL.StoredPrograms
...
/-- `stepCount n` is the number of checker steps of the n-node RTL program: 15 plus 68 per node. -/
def stepCount (n : Nat) : Nat := 68 * n + 15
...
end Quadrature.RTL.StoredPrograms
