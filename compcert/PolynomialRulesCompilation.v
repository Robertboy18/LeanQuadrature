...
From compcert Require Import Errors Integers Values Events Globalenvs Smallstep Behaviors
  ...
  Compiler Asm.
From QuadratureC Require Import ClightRules PolynomialRules AnnotatedBehavior
  CminorApplication CompilerBackend PolynomialRulesAccuracy.
...
Definition first_translation := Cshmgen.transl_program (App.application r).
...
Theorem first_program_translation :
  Cshmgen.transl_program (App.application r) = OK csharpminor_program.
...
Theorem application_forward_simulation :
  forward_simulation (Clight.semantics2 (App.application r)) (Cminor.semantics cminor_program).
Proof.
...
Theorem application_terminates :
  program_behaves (Cminor.semantics cminor_program) (Terminates (App.result_trace r) Int.zero).
Proof.
  eapply forward_simulation_termination.
  - exact application_forward_simulation.
  - exact (App.main_initial_state r).
  - exact (App.main_steps r).
  - constructor.
Qed.

Theorem application_all_behaviors beh :
  program_behaves (Cminor.semantics cminor_program) beh ->
  beh = Terminates (App.result_trace r) Int.zero.
Proof.
...
Proof.
  ...
  exact (result_behavior_accurate r).
Qed.
...
Theorem assembly_forward_simulation :
  forward_simulation (Clight.semantics2 (App.application r))
...
Theorem assembly_terminates :
  program_behaves (Asm.semantics assembly_program)
    (Terminates (App.result_trace r) Int.zero).
Proof.
  eapply forward_simulation_termination.
