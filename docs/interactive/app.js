  ...
  const proofs = [
    {
      ...
      href: "../../Quadrature/Analysis/Remainder.lean", link: "Corrected remainder theorem"
    },
    {
    ...
    },
    {
      ...
      scope: "Lean also proves that normalization preserves returned values, traces, termination, and caller memory for the six selected functions. Both frontend outputs initialize and resolve the actual calls. General parser and elaborator correctness and correspondence with Rocq’s definitions remain open. Separate authored Clight applications have proofs through formal assembly; native linking and printing remain outside them.",
    ...
    },
    {
      ...
      example: "In Rocq, all ten stored orders of the internal-polynomial application have checked compilation success, termination, unique annotated output, and integral-error bounds. Lean also proves total correctness through formal assembly, with concrete stack frames and executed calls and returns. The source and representation connections, instruction expansion, linking, and printing remain open.",
    ...
    if (state.route === "lean") {
      $("#route-content").innerHTML = String.raw`
        ...
        <div class="route-diagram">
          ...
          <div class="route-node"><p class="eyebrow">Lean · formal assembly proved</p><h3>Instructions, calls, and returns</h3><div class="route-equation">\(\lvert v_{\mathrm{asm}}-I\rvert\leq\varepsilon\)</div><p>All ten imported assembly programs have total-correctness proofs. The program counter follows calls and returns through concrete stack memory. Every execution emits the certified value and exits zero. Source and representation connections, native assembly/linking, and printing remain open.</p></div>
        </div>
        <p class="route-footnote"><strong>The selected C library also has its own total-correctness proof.</strong> Every permitted evaluation order terminates with the certified FloatLib result, using the original functions and verified C cosine. See <a href="#source-total-correctness">why every order is covered</a> and <a href="#source-normalization">how Clight normalization preserves the selected calls</a>. The diagram shows the separate authored Clight applications and their imported programs through formal assembly. Those stages have the same possible final observations, with an explicit source external-call determinism condition when composing the Clight result. The observation is an annotation; general parser and elaborator correctness, representation correspondence, instruction expansion, printing, and linking need further work. <a href="../../evidence/lean-quality.json">Lean build and audit ↗</a></p>`;
