...
result in our Lean adaptation of CompCert's C semantics. We also prove
that these calls preserve their results when translated to Clight. Separate
application proofs reach formal assembly in Lean, while independent Rocq
proofs use CompCert's compiler theorem. Connecting the C library to those
assembly proofs, proving general correctness of the source translation,
and establishing agreement between the Lean and Rocq definitions of
...
Appendix~\ref{sec:stewart-map} compares all 24 numbered paragraphs of their
mathematical presentation with the LeanPDE revision we use and the proofs
added here.
...
Clight, CompCert's simpler intermediate language for C. Separate proofs
establish the results of ten application programs through formal assembly
in Lean. We also prove those application results independently in Rocq,
where they compose with CompCert's compiler theorem.

These results leave a specific composition problem. The C proof covers the
initialized library; the assembly proofs start from separately written
Clight applications. We have not connected those applications to the
translated library. General correctness of the source translation and a
proof that the adapted Lean execution rules agree with Rocq's also remain
open. The comparison below separates the proved results from these
...
unproved. &
\textbf{mathlib, LeanPDE}, with our general quadrature proofs.
...
place of the external cosine. The C library and assembly application proofs
are separate results; a theorem connecting them remains open.}
\label{tab:comparison}
\end{table}

LeanPDE is a substantive dependency of these proofs. We reuse its
quadrature interfaces, its two-point Gauss--Legendre rule and cubic
exactness theorem, and its stability bounds under uniform approximation.
Its certified root brackets, rational interval arithmetic, Taylor
inequalities, and integration tactics establish the concrete enclosures.
The same LeanPDE revision also contains a sharp fourth-derivative error
bound for the two-point rule, proved with a Peano kernel. That result was
already available; our general remainder proof does not use it. We build
the general weighted construction, integrated remainder, and convergence
proof on mathlib. FloatLib supplies the binary64 operations and rounding
theorems used to connect those real bounds to computation.
...
of their roots by a counting argument, and derive the recurrence afterward.
...
polynomial approximation. The paragraph-by-paragraph comparison in
Appendix~\ref{sec:stewart-map} separates these additions from LeanPDE's
existing two-point development.
...
whose \code{clightgen} can emit Clight syntax as Lean, together with a
port of the Clight semantics. We adapted it in two ways. The exported
semantics used host floating-point operations. We replaced these with
...
Lean also has a reusable separation-logic framework.
Iris-Lean~\cite{irislean} provides the Iris base logic, a
...
In particular, direct execution proofs for these functions do not provide
VST's support for separately verified modules, higher-order function-pointer
specifications, or concurrency. A C program can implement its own object
system with function pointers and hidden mutable state. Scaling to such
programs calls for compositional specifications and proof automation,
beyond the operational semantics and application lemmas proved here.
...
VST supplies a much larger program logic and automation library. A
C-specific interface built on a framework such as Iris-Lean could reduce
the work of verifying further functions.
...
\begin{tikzpicture}[
 ...
 \draw[arrow,dashed,opencolor] (lc) --
   node[right,note]{\openproof{}\\library-to-application\\connection} (la);
...
proofs support both the initialized C library and the separate applications
checked through assembly. The right column describes our independent Rocq
proofs. Both columns end at formal assembly syntax, before native assembly
and linking.}
...
LeanQuadrature~\cite{leanquadrature} contains the proofs, the build
...
CompCert and Flocq. The project contains roughly 53,000 lines of Lean, a
...
computation executable inside the logic. The new work builds on these
libraries to prove the weighted construction, remainder, convergence, and
the accuracy of the ten stored rules. LeanPDE is essential to this
implementation because the proofs reuse those definitions and theorems.
Its existing sharp two-point error bound must be distinguished from our
general remainder theorem.
...
theorem in the same proof system. Our Lean development supplies direct
execution proofs for this library and these compiled applications.
Iris-Lean provides a reusable program-logic foundation, but applying it
here would require a C/Clight language instance and verified rules for
memory operations and calls, together with practical automation.
The current application proofs do not establish that direct execution
reasoning alone will scale to large modular C programs.
...
To join the current Lean results into a theorem from source to formal
assembly, the next application proof must connect the translated C library
to the Clight entry points used by the assembly certificates. A general
source-translation theorem would also have to justify parsing and
elaboration, including preprocessing and static-array handling. Reusing
...
\FloatBarrier
\appendix
\section{Stewart's 24 paragraphs}
\label{sec:stewart-map}

Appel and Bindel's Section 2 follows the numbered paragraphs of Chapter 23
of Stewart~\cite{stewart}. The PDF abbreviates several paragraphs and skips
14; the accompanying \code{quadrature.v} comments include its root
argument. The table compares their mathematical content with the LeanPDE
revision used by this project. The general results below assume a compact
interval, a positive continuous weight, and a positive node count.
A proved result need not follow Stewart's proof.

LeanPDE already supplied a normalized quadrature interface, the two-point
Gauss--Legendre rule, polynomial exactness through degree three, affine
transport to $[a,b]$, and stability under uniform perturbations. It also
supplied the sharp estimate
\[
...
in LeanPDE's \code{GaussLegendrePeano} module. Its Peano-kernel proof is
preexisting work, distinct from the general Hermite remainder proved here.
The companion
\href{https://github.com/Robertboy18/LeanQuadrature/blob/main/docs/stewart-comparison.md}{source comparison}
gives exact declarations, file links, and dependency revisions.
...
\caption{The 24 paragraphs, inherited results, and additions.
Definitions and alternative arguments are identified explicitly;
paragraphs 6--7 are not counted as separately formalized lemmas.}
\label{tab:stewart}\\
\toprule
\rowcolor{tablehead}
\P & Content & Already in LeanPDE & LeanQuadrature\\
\midrule
\endfirsthead
\toprule
\rowcolor{tablehead}
\P & Content & Already in LeanPDE & LeanQuadrature (continued)\\
\midrule
\endhead
\bottomrule
\endfoot
1 & Weighted quadrature sum &
Finite normalized rules and their evaluation; no general weighted Gaussian construction. &
\checked{Defined}: \code{GaussianRule} and its weighted sum on a compact interval.\\
\addlinespace
2 & Weighted integral notation &
Uses mathlib's interval integral; the normalized rule targets unit weight. &
\checked{Defined}: \code{polynomialIntegral} integrates a polynomial against $w$.\\
\addlinespace
3 & Linearity &
Linearity of rule evaluation and mathlib integration. &
\checked{Inherited and specialized}: the weighted polynomial integral is a linear map.\\
\addlinespace
4 & Orthogonality &
No general quadrature inner product. &
\proved{}: \code{momentInnerCore} gives the polynomial inner product from a positive integral.\\
\addlinespace
5 & Orthogonal polynomial family &
No general family in the quadrature development. &
\proved{}: the unique monic polynomial of each degree, with lower-degree orthogonality.\\
\addlinespace
6 & Expansion in a monic family &
No corresponding quadrature lemma. &
No separate basis-expansion theorem for an arbitrary monic family. Orthogonal projection supplies the construction directly.\\
\addlinespace
7 & Inductive expansion proof &
No corresponding quadrature proof. &
No separate proof of paragraph 6. \code{degree_sub_coeff_mul_lt} supplies degree reduction where the recurrence needs it.\\
\addlinespace
8 & Orthogonality to every lower degree &
No general Gaussian-family theorem. &
\proved{}: \code{orthogonalPolynomial_orthogonal} for the constructed family.\\
\addlinespace
9 & The first two monic polynomials &
Explicit two-point nodes, without this general family construction. &
\proved{}: \code{orthogonalPolynomial_zero} and \code{orthogonalPolynomial_one}.\\
\addlinespace
10 & Recurrence coefficients &
No general recurrence coefficients. &
\proved{}: \code{recurrenceAlpha}, \code{recurrenceBeta}, and the recurrence with those coefficients.\\
\addlinespace
11 & Vanishing lower terms &
No general recurrence proof. &
\proved{}: the three-term identity has no further lower terms. The proof uses lower-degree orthogonality.\\
\addlinespace
12 & Three-term recurrence &
No general recurrence theorem. &
\proved{}: \code{orthogonalPolynomial_recurrence}, with the positive norm-ratio identity.\\
\addlinespace
13 & Real, simple, interior roots &
The two explicit nodes lie in the interval. &
\proved{}: splitting over $\R$, squarefreeness, and strict interior membership at every positive order.\\
\addlinespace
14 & Root argument &
No general root argument. &
\checked{Alternative proof}: factorization, constant sign, and positive integrals of squares give the root results.\\
\addlinespace
15 & Weights from Lagrange polynomials &
General exactness from moments and an explicit two-point rule. &
\proved{}: \code{interpolatoryWeight}, interpolation exactness, and identification of Gaussian weights.\\
\addlinespace
16 & Degree $2n-1$ exactness for $n$ nodes &
Two-point exactness through degree three. &
\proved{}: \code{gaussian_exact} uses polynomial division at every positive order.\\
\addlinespace
17 & Positive quadrature weights &
The two weights are explicitly $1/2$ on $[0,1]$. &
\proved{}: \code{gaussian_weight_pos} from the integral of a squared Lagrange polynomial.\\
\addlinespace
18 & Total weight and weight bound &
Two-point total variation equals one on $[0,1]$. &
\proved{}: \code{sum_weights} and \code{weight_le_mass}.
The bound is $\int w$, already correct in the Rocq statement; the prose's bound of one requires normalization.\\
\addlinespace
19 & Gaussian remainder &
The sharp two-point fourth-derivative bound above. &
\proved{}: first-derivative Hermite interpolation and the integrated $2n$-derivative remainder, with explicit regularity.\\
\addlinespace
20 & Convergence for continuous functions &
Uniform-perturbation bounds and composite two-point methods; no general limit as Gaussian order grows. &
\proved{}: \code{gaussian_rules_converge}. A separate theorem covers positive normalized rules of increasing exactness.\\
\addlinespace
21 & Gauss--Legendre specialization &
The normalized two-point rule, affine transport, exactness, and its sharp error bound. &
\proved{}: the general compact theory specialized to unit weight; explicit rules through four nodes are identified with the monic family. Stored-table certificates cover ten orders.\\
\addlinespace
22 & Gauss--Laguerre on $[0,\infty)$ &
Outside this quadrature development. &
\openproof{}: no infinite-interval Gaussian construction or associated remainder and convergence proof.\\
\addlinespace
23 & Gauss--Hermite on $\R$ &
Outside this quadrature development. &
\openproof{}: no infinite-interval Gaussian construction. Hermite interpolation in paragraph 19 is a different subject.\\
\addlinespace
24 & Other choices of weight and interval &
A concluding observation, not a theorem. &
Our general result permits other positive continuous weights on compact intervals. It does not implement every named Gaussian family.\\
\end{longtable}
\endgroup
