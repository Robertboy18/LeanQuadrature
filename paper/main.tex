...
LeanPDE provides the mathematical theory used by LeanQuadrature:
weighted Gaussian rules of every positive order, orthogonal polynomials,
interior roots, exactness, positive weights, recurrence, remainder, and
convergence. It also provides explicit Legendre rules through four nodes
and general root and weight certificate proofs. LeanQuadrature imports
these results to verify the stored tables and programs.

Table~\ref{tab:stewart} identifies the available definitions and theorems
for the 24 mathematical paragraphs in Appel and Bindel's Section 2,
which follows Chapter 23 of Stewart~\cite{stewart}. Their PDF abbreviates
several paragraphs and skips 14; the \code{quadrature.v} comments include
its root argument. The general LeanPDE results assume a compact interval,
a positive continuous weight, and a positive node count. The construction
uses orthogonal projection, so it does not require the arbitrary-family
expansion argument in paragraphs 6--7. The infinite-interval families in
paragraphs 22--23 lie outside this scope.
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.45cm}
  ...
  >{\raggedright\arraybackslash}p{11.45cm}@{}}
\caption{Current LeanPDE coverage of the 24 mathematical paragraphs.
Checkmarks identify available definitions and proved results;
the other entries describe the proof route or mathematical scope.}
\label{tab:stewart}\\
\toprule
\rowcolor{tablehead}
\P & Mathematical topic & LeanPDE result\\
\midrule
\endfirsthead
\toprule
\rowcolor{tablehead}
\P & Mathematical topic & LeanPDE result (continued)\\
\midrule
\endhead
\bottomrule
\endfoot
...
represent and evaluate a weighted Gaussian rule on a compact interval.\\
\addlinespace
2 & Weighted integral notation &
\checked{Defined}: \code{polynomialIntegral} integrates a polynomial against $w$.\\
\addlinespace
3 & Linearity &
\checked{Defined and proved}: the weighted polynomial integral is a linear map,
using mathlib's integral linearity. Rule evaluation is also linear.\\
\addlinespace
4 & Orthogonality &
\proved{}: \code{momentInnerCore} constructs the polynomial inner product;
\code{polynomialIntegral_square_pos} supplies positivity for the weighted integral.\\
\addlinespace
5 & Orthogonal polynomial family &
\proved{}: \code{exists_monic_orthogonal} constructs a monic polynomial
of each degree, orthogonal to every lower degree;
\code{orthogonalPolynomial_unique} proves uniqueness.\\
\addlinespace
6 & Expansion in a monic family &
The construction uses orthogonal projection directly. The expansion
theorem for an arbitrary monic family is not a separate library theorem.\\
\addlinespace
7 & Inductive expansion proof &
\code{degree_sub_coeff_mul_lt} proves the leading-coefficient elimination
used in the recurrence. The family itself is constructed by projection.\\
\addlinespace
...
including the degree-one coefficient as a ratio of integrals.\\
\addlinespace
...
\code{orthogonal_root_mem_Ioo}, at every positive order.\\
\addlinespace
14 & Root argument &
...
The bound is the total integral mass $\int w$; it equals one for normalized weights.\\
\addlinespace
19 & Gaussian remainder &
\proved{}: \code{hermite_remainder}, \code{GaussianRule.remainder},
and \code{GaussianRule.abs_error_le} give the $2n$-derivative identity and bound.\\
\addlinespace
...
\proved{}: the general compact theory specializes to unit weight.
Explicit rules through four nodes are identified with the monic Legendre
family. General root and weight certificates support the stored-table proofs.\\
\addlinespace
22 & Gauss--Laguerre on $[0,\infty)$ &
\openproof{}: this infinite-interval Gaussian construction, remainder,
and convergence theorem are outside the compact-interval development.\\
\addlinespace
23 & Gauss--Hermite on $\R$ &
\openproof{}: this infinite-interval quadrature construction is outside
the development. Hermite interpolation in paragraph 19 is a different subject.\\
\addlinespace
