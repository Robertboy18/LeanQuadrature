...
mathematical presentation with the original LeanPDE baseline and the proofs
developed here, including those now shared through LeanPDE.
...
The sharp fourth-derivative error bound for the two-point rule, proved with
a Peano kernel, was already present in the original LeanPDE baseline.
Our general remainder proof does not use it. We developed the general
weighted construction, integrated remainder, and convergence proof in
LeanQuadrature, building on mathlib. These proofs now live in LeanPDE's
\code{PDE.Symbolic.Continuum} namespace and are imported here.
FloatLib supplies the binary64 operations and rounding theorems used to
connect those real bounds to computation.
...
Appendix~\ref{sec:stewart-map} separates this project's general theory from
the two-point results already present in the original LeanPDE baseline.
...
computation executable inside the logic. In LeanQuadrature we developed the
weighted construction, remainder, and convergence proofs, now shared through
LeanPDE, and the accuracy certificates for the ten stored rules.
LeanPDE is essential to this
implementation because the proofs reuse those definitions and theorems.
Its preexisting sharp two-point error bound must be distinguished from our
...
argument. The table preserves the original comparison with an
\href{https://github.com/lean-dojo/LeanPDE/tree/03f523aff09fe3ff3069d54088770a264fc281f1}{earlier LeanPDE revision}.
We developed the general Gaussian theory in LeanQuadrature and subsequently
moved it into LeanPDE; this project now imports those shared proofs.
The columns record provenance, not gaps in the current dependency.
The general results below assume a compact
interval, a positive continuous weight, and a positive node count.
A proved result need not follow Stewart's proof.

That LeanPDE baseline supplied a normalized quadrature interface, the two-point
...
in LeanPDE's \code{GaussLegendrePeano} module. Its Peano-kernel proof predates
the general Hermite remainder developed in LeanQuadrature, now also shared
through LeanPDE.
The companion
...
\caption{The 24 paragraphs at the original LeanPDE baseline and the
subsequent LeanQuadrature development.
...
\toprule
\rowcolor{tablehead}
\P & Content & Original LeanPDE baseline & LeanQuadrature development\\
\midrule
\endfirsthead
\toprule
\rowcolor{tablehead}
\P & Content & Original LeanPDE baseline & LeanQuadrature development (continued)\\
\midrule
\endhead
\bottomrule
\endfoot
