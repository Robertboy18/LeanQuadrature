#!/usr/bin/env python3
"""Rebuild the pinned upstream quadrature VST proofs and record their assumptions.

Inputs: --source-dir (the upstream checkout at the pinned revision with its LAProof submodule),
--work-dir, --jobs, and the optional --type-scope-fix. Output: a new --output JSON record and a
sibling logs directory.
Requires the opam switch described in scripts/upstream/README.md. The REQUIRED_PACKAGES versions
are verified against `opam list` before anything is built.
"""
...
MATHCOMP_SINGLETON_INTEGRAL = (
    "mathcomp.analysis.lebesgue_integral_theory.lebesgue_Rintegral.Rintegral_set1"
)
HUGHES_SPECIFICATION = "CFEM.C.spec_quadrules.hughes_weight_spec"
WEIGHT_INITIALIZER = "CFEM.C.quadrules.v_gauss_wts"
ADMITTED_DIAGNOSTIC = (
    "UpstreamReview.MathematicalAudit.admitted_positivity_is_inconsistent"
)
REVIEW_BASELINES = {
    **{f"UpstreamReview.SpecAudit.{name}": () for name in REVIEW_LEMMAS},
    "UpstreamReview.SpecAudit.hughes_weight_identifier_mismatch": (HUGHES_SPECIFICATION,),
    "UpstreamReview.SpecAudit.three_point_initializer_model_mismatch": (WEIGHT_INITIALIZER,),
    "UpstreamReview.SpecAudit.semax_body_ignores_identifier": (VST_BODY_DEFINITION,),
    "UpstreamReview.SpecAudit.body_hughes_weight_corrected_identifier":
        ("CFEM.C.verif_quadrules.body_hughes_weight",),
    "UpstreamReview.MathematicalAudit.singleton_interval_positivity_counterexample":
        (MATHCOMP_SINGLETON_INTEGRAL,),
    ADMITTED_DIAGNOSTIC: ("CFEM.quadrature.Rintegral_gt_0", MATHCOMP_SINGLETON_INTEGRAL),
}
QUERIES += [
    *REVIEW_BASELINES, VST_BODY_DEFINITION, MATHCOMP_SINGLETON_INTEGRAL,
    HUGHES_SPECIFICATION, WEIGHT_INITIALIZER,
]
REVIEW_MODULES = ["SpecAudit", "MathematicalAudit"]
...
def scope_sigma_products(text):
    """Disambiguate exactly the two dependent product types in the pinned source."""
    original = "X: {n: 'I_5 & 'I_n * 'I_n}"
    scoped = "X: {n: 'I_5 & ('I_n * 'I_n)%type}"
    if text.count(original) != 2:
        raise ValueError("The two expected product types were not found")
    return text.replace(original, scoped)
...
def check_review_assumptions(assumptions):
    """Allow only assumptions inherited from each check's stated dependencies."""
    checks = {}
    for name, baselines in REVIEW_BASELINES.items():
        allowed = {
            axiom for baseline in baselines for axiom in assumptions[baseline]["axioms"]
        }
        actual = set(assumptions[name]["axioms"])
        unexpected = sorted(actual - allowed)
        checks[name] = {"baselines": list(baselines), "unexpected_axioms": unexpected}
        if unexpected:
            raise RuntimeError(f"The review lemma {name} adds axioms: {unexpected}")
        if name == ADMITTED_DIAGNOSTIC and actual != allowed:
            raise RuntimeError(
                "The diagnostic must retain the original admission and the "
                "singleton-integral theorem's logical assumptions"
            )
    return checks
...
def main():
    ...
    parser.add_argument(
        "--type-scope-fix", action="store_true",
        help="Build a separate source copy with two explicit product-type annotations",
    )
    ...
    try:
        ...
        build_source = source
        record["source_adjustment"] = None
        if args.type_scope_fix:
            build_source = work / "source"
            build_source.mkdir()
            for name, expected in record["inputs"].items():
                data = (source / name).read_bytes()
                if hashlib.sha256(data).hexdigest() != expected:
                    raise RuntimeError(f"The original source {name} changed before copying")
                destination = build_source / name
                destination.parent.mkdir(parents=True, exist_ok=True)
                destination.write_bytes(data)
            relative = "proof/C/spec_quadrules_highlevel.v"
            original = (build_source / relative).read_text()
            adjusted = scope_sigma_products(original)
            (build_source / relative).write_text(adjusted)
            patch = logs / "type-scope.diff"
            patch.write_text("".join(difflib.unified_diff(
                original.splitlines(keepends=True), adjusted.splitlines(keepends=True),
                fromfile=f"original/{relative}", tofile=f"adjusted/{relative}",
            )))
            record["source_adjustment"] = {
                "file": relative,
                "original_sha256": record["inputs"][relative],
                "adjusted_sha256": sha256(build_source / relative),
                "diff": str(patch.relative_to(output.parent)), "diff_sha256": sha256(patch),
                "scope": (
                    "Two explicit %type annotations in dependent product types. "
                    "No theorem proof, admitted lemma, or C source was edited. "
                    "The original checkout remains unchanged."
                ),
            }
        record["build_source_directory"] = str(build_source)
        record["build_inputs"] = {
            name: sha256(build_source / name) for name in record["inputs"]
        }
        driver = Path(__file__).resolve()
        templates = {
            name: driver.parent / "upstream" / f"{name}.v" for name in REVIEW_MODULES
        }
        review_inputs = {
            driver.name: driver,
            "upstream_sources.py": driver.parent / "upstream_sources.py",
            **{f"upstream/{name}.v": path for name, path in templates.items()},
        }
        record["review_sources"] = {
            name: sha256(path) for name, path in review_inputs.items()
        }
        review_snapshot = logs / "review-sources"
        for name, path in review_inputs.items():
            destination = review_snapshot / name
            destination.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(path, destination)
            if sha256(destination) != record["review_sources"][name]:
                raise RuntimeError(f"The review source {name} changed while being retained")
        record["review_source_directory"] = str(review_snapshot.relative_to(output.parent))
        installed = captured(
        ...
        project_sources = [
            *source_files(build_source / "LAProof" / "_CoqProject"),
            *source_files(build_source / "proof" / "_CoqProject"),
        ]
        ...
        record["absent_project_sources"] = [
            str(path.relative_to(build_source)) for path in project_sources if not path.is_file()
        ]
        sources = [path for path in project_sources if path.is_file()]
        record["preexisting_compiled_modules"] = [
            str(path.relative_to(build_source)) for path in sources if path.with_suffix(".vo").exists()
        ]
        project = work / "ReviewProject"
        project.write_text(
            f'-Q "{build_source / "LAProof"}" LAProof\n-Q "{build_source / "proof"}" CFEM\n'
        ...
        run(
            ["make", "-f", "ReviewMakefile", f"-j{args.jobs}",
             str(build_source / "proof" / "C" / "verif_quadrules.vo")],
            "original-build",
        )
        record["compiled_modules"] = {
            str(path.relative_to(build_source)): sha256(path.with_suffix(".vo"))
            for path in sources if path.with_suffix(".vo").exists()
        }

        review_dir = work / "review"
        review_dir.mkdir(exist_ok=True)
        flags = ["-Q", str(build_source / "LAProof"), "LAProof",
                 "-Q", str(build_source / "proof"), "CFEM",
                 "-Q", str(review_dir), "UpstreamReview"]
        for name, template in templates.items():
            shutil.copy2(template, review_dir / f"{name}.v")
            run(["coqc", *flags, str(review_dir / f"{name}.v")], f"review-{name}")
        assumption_dir = work / "assumptions"
        assumption_dir.mkdir(exist_ok=True)
        queries = review_dir / "Assumptions.v"
        queries.write_text(
            "From CFEM.C Require Import verif_quadrules.\n"
            "From UpstreamReview Require Import SpecAudit MathematicalAudit.\n"
            + "".join(
                f'Redirect "assumptions/{index:02d}" Print Assumptions {name}.\n'
                for index, name in enumerate(QUERIES)
            )
        )
        run(["coqc", *flags, str(queries)], "assumption-queries")
        record["assumptions"] = {}
        for index, name in enumerate(QUERIES):
        ...
        record["review_assumption_checks"] = check_review_assumptions(record["assumptions"])
        run(
            ["coqchk", *flags, "-norec", "CFEM.C.verif_quadrules",
             "-norec", "UpstreamReview.SpecAudit",
             "-norec", "UpstreamReview.MathematicalAudit"],
            "kernel-recheck",
        )
        record["kernel_recheck_scope"] = (
            "coqchk -norec checks the upstream body-proof module and both review modules. "
        ...
        if {
            name: sha256(build_source / name) for name in record["build_inputs"]
        } != record["build_inputs"]:
            raise RuntimeError("The sources used for compilation changed during the review")
        if {name: sha256(path) for name, path in review_inputs.items()} != record["review_sources"]:
            raise RuntimeError("Review sources changed during the run")
        for name in REVIEW_MODULES:
            if sha256(review_dir / f"{name}.v") != record["review_sources"][f"upstream/{name}.v"]:
                raise RuntimeError(f"The compiled review source {name} changed during the run")
