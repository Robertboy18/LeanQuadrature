#!/usr/bin/env python3
"""Reproduce the RTL import, the RTLgen output, from the checked configured CompCert build.

Inputs: --proof-build, --work-dir, --assembly-evidence. Output: the --output JSON record (the
retained run is evidence/rtl-import.json).
Requires the configured proof build and Docker with the pinned Coq image. The script loads
reproduce-cminor-import.py by path as BASE and scripts/import-rtl.py as the importer. The
reconstructed Rocq AST is checked against the actual compiler computation, and the Lean syntax
must reproduce Quadrature/Compiler/RTL/Imported.lean byte for byte.
"""
...
def main():
    ...
    (work / "RunExport.v").write_text(
        ...
        "From CminorImport Require Import ExportRTL.\n"
        "From compcert Require Import Errors Selection SelectOp Compopts RTLgen.\n"
        ...
        "Definition exported n := match translated n with\n"
    ...
    for name in ["ExportCminor.v", "ExportCminorSel.v", "EqualityCminorSel.v",
                 "ExportRTL.v", "EqualityRTL.v", "RunExport.v"]:
        compile_file(name)
    importer = load_module("rtl_import", REPO / "scripts/import-rtl.py")
    generated = importer.generate(work, work / "Imported.lean", work / "ImportedRTL.v")
    BASE.check_hash(work / "Imported.lean", BASE.digest(REPO / "Quadrature/Compiler/RTL/Imported.lean"))
    audit = compile_file("ImportedRTL.v")
    ...
    artifacts = [
        ...
        for name in ["ExportCminor", "ExportCminorSel", "EqualityCminorSel",
                     "ExportRTL", "EqualityRTL", "RunExport", "ImportedRTL"]
        for suffix in [".v", ".v.log", ".vo"]
    ] + ["Imported.lean"]
    tools = [
        "scripts/reproduce-cminor-import.py", "scripts/reproduce-rtl-import.py",
        "scripts/import-cminor.py", "scripts/import-rtl.py",
    ...
    record = {
        ...
        "kind": "Concrete RTL syntax checked against configured RTL generation",
        ...
        "lean_import_sha256": BASE.digest(REPO / "Quadrature/Compiler/RTL/Imported.lean"),
        ...
        "rocq_certificate_theorem": "ImportedRTL.imported_program_is_compiler_output",
        "rocq_certificate_axioms": sorted(axioms),
        "lean_syntax_reproduced_exactly": True,
        "lean_general_rtlgen_pass_proved": False,
